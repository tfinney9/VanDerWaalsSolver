...
shinyServer(function(input, output) {
  ...
  writeJob<-reactive({
    ...
    cfg<-paste("/srv/shiny-server/vdw/jData/proc.cfg")
    cat("[VDW Solver Job]\n",file=cfg)
    if(input$manual==1){
      if(input$abcr==1){ #A,B
        nLoc<-match(input$vdwspec,vdwSpecies[[1]])
  ...
  observeEvent(input$gogo,{
    ...
    resultFile<-read.csv(file="/srv/shiny-server/vdw/jData/result.txt")
  ...
  writeAnt<-reactive({
    ...
    cfg<-paste("/srv/shiny-server/vdw/jData/antX.cfg")
    cat("[Antoine CFG]\n",file=cfg)
  ...
  observeEvent(input$antoineGo,{
    ...
    ANresultFile<-read.csv(file="/srv/shiny-server/vdw/jData/anResult.txt")
